...
import pandas as pd
from utils.eval import MetricsDAG, postprocess
...
def miss_dag_nongaussian(X, dag_init_method, dag_method, em_iter=10, MLEScore='Sup-G', num_sampling=20, B_true=None):
    ...
    for m in range(1, em_iter + 1):
        ...
        print("Finished post-processing the estimated graph.")

        raw_result = MetricsDAG(B_processed_bin, B_true).metrics
        print("run result:{0}".format(raw_result))
        ################################################

        print("Started the {}th iteration for EM algorithm with DAG learning.".format(m))
